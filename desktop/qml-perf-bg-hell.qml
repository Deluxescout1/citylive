...
// COST OF THE BACKDROP PASS, hell against alpine. Everything the Ashlands gained — hashed crags, the rift
...
// Alternating the two lands inside ONE process removes per-run drift, and comparing hell/alpine rather than
// hell alone removes machine drift between the before and after runs as well.
// Run: QT_ASSUME_STDERR_HAS_CONSOLE=1 QT_QPA_PLATFORM=offscreen qml6 desktop/qml-perf-bg-hell.qml
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var tot = { hell: 0, alpine: 0 }, cnt = { hell: 0, alpine: 0 };
            ...
            for (var r = 0; r < 3; r++) {
                one("hell", 13); one("alpine", 13); one("alpine", 23); one("hell", 23);
            }
            var h = tot.hell / cnt.hell, a = tot.alpine / cnt.alpine;
            console.log("BGPERF hell=" + h.toFixed(2) + "ms alpine=" + a.toFixed(2)
