...
Item { id: root; width: 2269; height: 437
  property var shots: [
    { n:"alpine-calm",  wind:4,  cloud:10, hour:13 },
    { n:"alpine-windy", wind:26, cloud:45, hour:13 },
    { n:"alpine-night", wind:14, cloud:20, hour:22 }
  ]
  property int idx: 0
  property int warm: 0
  property double t0: 0
  property string outDir: "/tmp/claude-1000/-home-deluxescout/48f22420-cb8d-47ef-8c39-d5207bc423d6/scratchpad/alpine"
  function arm(){
    var S=root.shots[root.idx], CYC=604800000, EPOCH=1783972450746;
    City.GROW_CYCLE=CYC; City.NOFETCH=true; City.FORCEEGG=null;
    City.FORCEBIOME='alpine'; City.FORCEVARIANT=0;          // pin the BASE look, the one that was null
    var d=new Date(EPOCH+19*CYC+Math.round(0.45*CYC)); d.setHours(S.hour,0,0,0);
    ...
    City.FORCEAGE=0.85;
    City.weather.code=0; City.weather.wind=S.wind; City.weather.temp=34; City.weather.cloud=S.cloud;
  }
