...
Item { id: root; width: 854; height: 480
  ...
  function arm(){
    ...
    City.applyConfig({lat:41.5243,lon:-72.0759, land:"alpine", landVariant:0});
    var d=new Date(1783972450746+360*3600000+Math.round(0.5*3600000)); d.setHours(14,0,0,0);
    ...
    City.FORCEAGE=0.5; City.weather.code=0; City.weather.wind=6; City.weather.temp=44;
  }
  ...
  Timer { interval:340; running:true; repeat:true
    onTriggered:{ if(root.done){Qt.quit();return}
      ...
      root.grabToImage(function(r){ r.saveToFile(root.outDir+"/ibex.png"); root.done=true; }); } }
}
