...
// SPACE CITY through one 90-minute orbit — the point of the land is that its clock is the ORBIT.
Item { id: root; width: 854; height: 480
  property var fr: [0.05, 0.28, 0.52, 0.78]
  property int idx: 0
  property int warm: 0
  property double t0: 0
  property string outDir: "/tmp/claude-1000/-home-deluxescout/2db058e7-bff3-40ca-bb41-421151bb5164/scratchpad/orbit"
  function arm(){
    var CYC=604800000, EPOCH=1783972450746;
    City.GROW_CYCLE=CYC; City.NOFETCH=true;
    City.FORCEBIOME=null; City.FORCEVARIANT=null; City.FORCEEGG='orbit';
    var d=new Date(EPOCH+44*CYC+Math.round(0.45*CYC)); d.setHours(13,0,0,0);
    var base=d.getTime();
    root.t0 = base - (base % 5400000) + Math.round(root.fr[root.idx]*5400000);
  ...
  Timer { interval:340; running:true; repeat:true
    onTriggered:{ if(root.idx>=root.fr.length){Qt.quit();return}
      ...
      root.grabToImage(function(r){ r.saveToFile(root.outDir+"/orbit-"+root.idx+".png"); root.idx++; root.warm=0; }); } }
}
