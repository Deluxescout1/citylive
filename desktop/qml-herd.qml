...
// The big grazers on the plains, two frames apart so the gait is visible.
Item { id: root; width: 854; height: 480
  property var ts: [0, 240]
  property int idx: 0
  property int warm: 0
  ...
  function arm(){
    ...
    var d=new Date(1783972450746+44*604800000+Math.round(0.30*604800000)); d.setHours(14,0,0,0);
    root.t0=d.getTime()+root.ts[root.idx];
    City.NOWOVR=City.CLOCK=root.t0;
    City.applyConfig({lat:41.5243,lon:-72.0759});
    City.setup('neon',{cw:854,ch:480,woff:400,ww:2269,pxk:3,zoom:1,taskbarWp:28,quality:'balanced',frameMs:125});
    City.FORCEAGE=0.22; City.weather.code=0; City.weather.wind=7; City.weather.temp=64;
  }
  ...
  Timer { interval:340; running:true; repeat:true
    onTriggered:{ if(root.idx>=root.ts.length){Qt.quit();return}
      ...
      root.grabToImage(function(r){ r.saveToFile(root.outDir+"/herd-"+root.idx+".png"); root.idx++; root.warm=0; }); } }
}
