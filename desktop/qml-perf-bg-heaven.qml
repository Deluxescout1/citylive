...
// COST OF THE BACKDROP PASS, heaven against alpine. Everything the Ashlands gained — hashed crags, the rift
...
// Alternating the two lands inside ONE process removes per-run drift, and comparing heaven/alpine rather than
// heaven alone removes machine drift between the before and after runs as well.
// Run: QT_ASSUME_STDERR_HAS_CONSOLE=1 QT_QPA_PLATFORM=offscreen qml6 desktop/qml-perf-bg-heaven.qml
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var tot = { heaven: 0, alpine: 0 }, cnt = { heaven: 0, alpine: 0 };
            ...
            for (var r = 0; r < 3; r++) {
                one("heaven", 13); one("alpine", 13); one("alpine", 23); one("heaven", 23);
            }
            var h = tot.heaven / cnt.heaven, a = tot.alpine / cnt.alpine;
            console.log("BGPERF heaven=" + h.toFixed(2) + "ms alpine=" + a.toFixed(2)
