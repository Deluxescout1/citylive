...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var lands = ["alpine","savanna"];
            var tot = {}, cnt = {}, totb = {}, cntb = {};
            ...
            function one(land, hour) {
                ...
                var tb = Date.now();
                for (var kb = 0; kb < 6; kb++) { City.savCache=null; City.NOWOVR = City.CLOCK = d.getTime() + kb*140; City.draw(g, "bg"); }
                totb[land] = (totb[land]||0) + (Date.now() - tb); cntb[land] = (cntb[land]||0) + 6;
            }
            for (var r = 0; r < 3; r++)
            ...
            for (var m = 0; m < lands.length; m++) {
                ...
                out += "\n  " + lands[m] + " live=" + v.toFixed(2) + "ms x" + (v/base).toFixed(2)
                     + "   bg=" + (totb[lands[m]]/cntb[lands[m]]).toFixed(2) + "ms";
            }
            console.log(out);
