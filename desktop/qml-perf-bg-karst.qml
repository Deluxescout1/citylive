...
// COST OF THE BACKDROP PASS, karst against alpine. Everything the Ashlands gained — hashed crags, the rift
...
// Alternating the two lands inside ONE process removes per-run drift, and comparing karst/alpine rather than
// karst alone removes machine drift between the before and after runs as well.
// Run: QT_ASSUME_STDERR_HAS_CONSOLE=1 QT_QPA_PLATFORM=offscreen qml6 desktop/qml-perf-bg-karst.qml
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var tot = { karst: 0, alpine: 0 }, cnt = { karst: 0, alpine: 0 };
            ...
            for (var r = 0; r < 3; r++) {
                one("karst", 13); one("alpine", 13); one("alpine", 23); one("karst", 23);
            }
            var h = tot.karst / cnt.karst, a = tot.alpine / cnt.alpine;
            console.log("BGPERF karst=" + h.toFixed(2) + "ms alpine=" + a.toFixed(2)
