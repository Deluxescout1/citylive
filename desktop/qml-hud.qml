...
Item { id: root; width: 854; height: 480
  property var lands: ["canyon","dunes","under","savanna","sprawl","hell"]
  property int idx: 0
  property int warm: 0
  property double t0: 0
  property string outDir: "/tmp/claude-1000/-home-deluxescout/2db058e7-bff3-40ca-bb41-421151bb5164/scratchpad/hud"
  function arm(){
    var CYC=604800000, EPOCH=1783972450746;
    City.GROW_CYCLE=CYC; City.NOFETCH=true; City.FORCEEGG=null;
    City.FORCEBIOME=root.lands[root.idx]; City.FORCEVARIANT=0;
    ...
    City.FORCEAGE=0.6; City.weather.code=0; City.weather.wind=7; City.weather.temp=64;
  }
  ...
  Timer { interval:340; running:true; repeat:true
    onTriggered:{ if(root.idx>=root.lands.length){Qt.quit();return}
      if(root.warm===0) root.arm();
      bg.requestPaint(); live.requestPaint();
      if(root.warm<1){root.warm++;return}
      root.grabToImage(function(r){ r.saveToFile(root.outDir+"/hud-"+root.lands[root.idx]+".png"); root.idx++; root.warm=0; }); } }
}
