...
// COST OF THE BACKDROP PASS, dunes against alpine. Everything the Ashlands gained — hashed crags, the rift
...
// Alternating the two lands inside ONE process removes per-run drift, and comparing dunes/alpine rather than
// dunes alone removes machine drift between the before and after runs as well.
// Run: QT_ASSUME_STDERR_HAS_CONSOLE=1 QT_QPA_PLATFORM=offscreen qml6 desktop/qml-perf-bg-dunes.qml
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var tot = { dunes: 0, alpine: 0 }, cnt = { dunes: 0, alpine: 0 };
            ...
            for (var r = 0; r < 3; r++) {
                one("dunes", 13); one("alpine", 13); one("alpine", 23); one("dunes", 23);
            }
            var h = tot.dunes / cnt.dunes, a = tot.alpine / cnt.alpine;
            console.log("BGPERF dunes=" + h.toFixed(2) + "ms alpine=" + a.toFixed(2)
